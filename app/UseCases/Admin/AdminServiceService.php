...
class AdminServiceService
{
    ...
    public function approveService(int $id, UploadedFile $documentoSubgerencia, int $approverId): void
    {
        ...
        DB::transaction(function () use ($servicio, $documentoPath, $approverId, &$publicoId) {
            ...
            $publicoId = DB::table('servicios_publicos')->insertGetId([
        ...
        if (!empty($servicio->correo_electronico) && $publicoId) {
            try {
                Mail::to($servicio->correo_electronico)->send(new EmpresaPublicacionAprobada([
                    'nombre_empresa' => $servicio->nombre_empresa,
                    'titulo' => $servicio->nombre_servicio,
                    'descripcion' => $servicio->descripcion,
                    'url' => url('/publicidad/servicios'),
                    'tipo' => 'servicio',
                    'documento_aprobacion_pdf' => $documentoPath,
                ]));
            } catch (\Exception $e) {
    ...
    public function rejectService(int $id, string $motivoRechazo, int $rejectorId): void
    {
        ...
        if (!empty($servicio->correo_electronico)) {
            try {
                Mail::to($servicio->correo_electronico)->send(new PublicacionRechazada([
                    'nombre_empresa' => $servicio->nombre_empresa,
                    'titulo' => $servicio->nombre_servicio,
                    'descripcion' => $servicio->descripcion,
                    'motivo' => $motivoRechazo,
                    'url' => url('/registro/servicios'),
                    'tipo' => 'servicio',
                ]));
            } catch (\Exception $e) {
