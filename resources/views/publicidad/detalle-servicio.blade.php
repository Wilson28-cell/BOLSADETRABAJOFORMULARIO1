...
@section('title', $servicio->nombre_servicio ?? 'Detalle de servicio')

@section('content')
@php
    use Illuminate\Support\Str;

    $serviceName = trim((string) ($servicio->nombre_servicio ?? ''));
    $companyName = trim((string) ($servicio->nombre_empresa ?? ''));
    $category = trim((string) ($servicio->categoria ?? ''));
    $description = trim((string) ($servicio->descripcion ?? ''));

    $rawImages = $servicio->imagen_servicio ?? null;
    $images = [];
    if (!empty($rawImages)) {
    ...
    $imageUrls = [];
    $buildImageUrl = static function ($path) use (&$imageUrls) {
    ...
    $rawSocial = $servicio->redes_sociales ?? null;
    $socialLinks = [];
    if (!empty($rawSocial)) {
    ...
    $detailItems = [];
    $appendDetailItem = static function ($field, $label, $icon, $formatter = null) use (&$detailItems, $servicio) {
    ...
    $contactEmail = trim((string) ($servicio->correo_contacto ?? $servicio->correo_electronico ?? ''));
    $contactPhone = trim((string) ($servicio->telefono_contacto ?? $servicio->telefono ?? ''));

    $whatsappValue = trim((string) (data_get($servicio, 'whatsapp') ?? data_get($servicio, 'whatsapp_contacto') ?? data_get($servicio, 'numero_whatsapp') ?? data_get($servicio, 'whatsapp_numero') ?? ''));
...
<div class="service-detail-shell">
    <div class="container py-5">
        <a href="{{ url('publicidad/servicios') }}" class="btn btn-outline-secondary rounded-pill px-4 py-2 mb-4">
            <span class="me-2">←</span> Volver a servicios
        </a>
                        ...
                        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start gap-3 mb-4">
                            <div>
                                <div class="d-flex flex-wrap gap-2 mb-3">
                                    <span class="badge service-badge">Plataforma de servicios</span>
                                    @if($category !== '')
                                        <span class="badge text-bg-light text-primary rounded-pill px-3 py-2">{{ e($category) }}</span>
                                    @endif
                                </div>
                                <h1 class="service-title mb-2">{{ $serviceName !== '' ? e($serviceName) : 'Servicio sin nombre' }}</h1>
                            ...
                            <div class="gallery-card mb-4">
                                @if($hasGallery)
                                    <div id="gallery-stage" class="gallery-stage" data-images='@json($galleryImages)'>
                                        <img id="gallery-main-image" src="{{ $primaryImage }}" alt="{{ e($serviceName) }}">
                                ...
                                @else
                                    <div class="gallery-stage single-image">
                                        <img src="{{ $primaryImage }}" alt="{{ e($serviceName) }}">
                                    </div>
                                @endif
                            </div>
                        @else
                            <div class="gallery-placeholder mb-4">
                                ...
                                <p class="mb-0">Este servicio aún no tiene imágenes registradas.</p>
                            </div>
                        @endif
        ...
        <div class="institutional-footer mt-4">
            <p class="mb-0">Este servicio pertenece a la Plataforma de Promoción de Productos y Servicios de la Municipalidad Distrital de El Porvenir.</p>
        </div>
    </div>
</div>
