...
use App\Mail\EmpresaPublicacionAprobada;
use App\Mail\PublicacionRechazada;
...
class AdminProductService
{
    ...
    public function approveProduct(int $id, ?UploadedFile $documentoSubgerencia = null): void
    {
        ...
        DB::transaction(function () use ($producto, $nuevoDocumento, $nuevaImagen, $documentoSubgerenciaPath, $id, &$publicoId) {
            ...
            $publicoId = DB::table('productos_publicos')->insertGetId([
        ...
        if (!empty($producto->correo_electronico) && $publicoId) {
            try {
                Mail::to($producto->correo_electronico)->send(new EmpresaPublicacionAprobada([
                    'nombre_empresa' => $producto->nombre_empresa,
                    'titulo' => $producto->nombre_producto,
                    'descripcion' => $producto->descripcion,
                    'url' => url('/publicidad/productos/' . $publicoId),
                    'tipo' => 'producto',
                    'documento_aprobacion_pdf' => $documentoSubgerenciaPath,
                ]));
            } catch (\Exception $e) {
                // Ignorar fallo de correo para no interrumpir la aprobación.
            }
        }
    }

    public function rejectProduct(int $id, ?string $motivo = null): void
    {
        $producto = $this->getPendingProduct($id);
        if (!$producto) {
            throw new \RuntimeException('Producto pendiente no encontrado.');
        ...
        DB::transaction(function () use ($producto, $motivo) {
            if (Schema::hasTable('empresas_producto_rechazadas')) {
                $rechazadoData = [
                    'id_empresa_original' => $producto->id_empresa_producto,
                    'id_producto_original' => $producto->id_producto,
                ...
                if (Schema::hasColumn('empresas_producto_rechazadas', 'motivo_rechazo')) {
                    $rechazadoData['motivo_rechazo'] = trim($motivo ?? '');
                }
                ...
                // Eliminar registros pendientes para que no sigan apareciendo en formularios
                ...
                // agregar motivo si existe la columna en productos_rechazados
                if (Schema::hasColumn('productos_rechazados', 'motivo_rechazo')) {
                    $rechazadoData['motivo_rechazo'] = $motivo ?? '';
                }
        ...
        if (!empty($producto->correo_electronico)) {
            try {
                Mail::to($producto->correo_electronico)->send(new PublicacionRechazada([
                    'nombre_empresa' => $producto->nombre_empresa,
                    'titulo' => $producto->nombre_producto,
                    'descripcion' => $producto->descripcion,
                    'url' => url('/registro/productos'),
                    'tipo' => 'producto',
                    'motivo' => $motivo ?? null,
                ]));
            } catch (\Exception $e) {
                // Ignorar fallo de correo para no interrumpir el rechazo.
            }
        }    }
